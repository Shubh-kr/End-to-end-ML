...
class Pipleine:
    def __init__(self, rs=None, compare_scaling_effect=True, show=False):
        ...
        self.time_metadata = {}
        self.randomm_state = rs
        self.show = show
        self.compare_scaling_effect = compare_scaling_effect

    def run_pipeline(self):
        """
        Runs a machine leanring pipleine from getting data until generating 
        ...
        print(f'{" PIPELINE ENDS ":^42s}')

    def get_data(self):
        """
        Gets the data from a remote server (UCI ML Reporsitory) and loads it
        ...
        try:
            self.col_names = ['sepal_length', 'sepal_width', 'petal_legth', 'petal_width', 'class']
    ...
    def prepare_data(self):
        """
        Preprocess the data by encoding the class labels, splitting the data train and validation sets, and scaling the 
        training data.
        """
    ...
    def _encode_labels(self):
        """
        Encode the class labels in the DataFrame.
        """
    ...
    def _feature_engineering(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        Returns:
        - (pd.DDataFrame): data with feature engineering only.
        """
        ...
        #take only new features for training
    ...
    def _visualize_data(self, title: str):
        """
        Functions to plot features for scaling effect comparison purpose
        """
    ...
    def build_model(self):
        """
        Build Machine Learning Model (3 model variaion)
        """
        ...
        self.time_metadata["Build Midel Time"] = f'{round((end-start), 2)} s'
    ...
    def export_file(self):
        ...
        """
        print(f'{"Exporting Piepline File ":.<28s}', end=' ')
        start = time.time()
        ...
        joblib.dump(self.pipeline_object, './model.bin')
    ...
    def generate_report(self):
        """
        Genrates report of the validation results, including metrics comparison
        & confusin matrices. The report is saved as an excel file and a PNG image.
        """
        ...
        report_dir = 'reesult'
        if not os.path.existis(report_dir):
        ...
        plt.rcParams['ytick.labelzise'] = 18.5
        ...
        ax2.legend(shadown=True, frameon=True)

        #confusion matrix
        ax40 = fig.add_subplot(gs[:,0])
        ...
        df_obj_info = pd.DatFrame().from_dict(obj_info, oreint='index', columns=['Object Type'])
        ...
        #exporting validation report and object info
        with pd.ExcelWriter(f'{report_dir}/{str(datetime.now().date()).replace("-", "_")}_pipeline_results.xlsx') as writer:
            df_obj_info.to_excel(writer, sheet_name='trained_data_statistics')
        ...
        print(f'done ({self.time_metadata["Generate File Time"]})')
